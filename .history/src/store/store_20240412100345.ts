...
const inputStore = create((set) => ({
    ...
    addFundraisingTimeline:(fundraisingTimeline:number)=>set((state:any)=>({fundraisingTimeline})),addCashBalance:(cashBalance:number)=>set((state:any)=>({cashBalance})),
    addRunwayMonths:(runwayMonths:number)=>set((state:any)=>({runwayMonths})),
