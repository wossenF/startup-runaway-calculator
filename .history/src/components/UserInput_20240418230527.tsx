...
const UserInput = () => {
  ...
  const handleInputChange = (fieldName:number) => (e) => {
    const value = e.target.value;
    validateField(fieldName, value);
    setField(fieldName, parseFloat(value));
  };

  const validateField = async (fieldName: number, value: any) => {
    try {
      await validationSchema.validateAt(fieldName :string , { [fieldName]: value });
