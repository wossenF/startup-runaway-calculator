...
import { calculateRunway } from '../utils/calculations'; // Import your calculation function
...
const UserInput = () => {
  ...
  const { initialCashBalance, monthlyIncome, monthlyGrowthRate, cogsPercentage, payRoll, nonPayRoll } = useInputStore();
  ...
  const calculateRunaway = () => {
    const userInput = {
      ...
      payRoll,
      nonPayRoll,
    fundraisingTimeline: 0, // Add appropriate value
    payrollReductionTimeline: 0, // Add appropriate value
    payrollReduction: 0, // Add appropriate value
    nonPayrollReductionTimeline: 0, // Add appropriate value
    nonPayrollReduction: 0, // Add appropriate value
    monthlyCompensation: 0, // Add appropriate value
    fundraisingAmount: 0, // Add appropriate value
    };
    const runaway = calculateRunway(userInput);
    setRunawayMonths(Number(runaway));
    setIsClicked(true);
  }
  ...
  return (
    <>
      ...
      <Button onClick={calculateRunaway}>{isClicked ? 'Back to Calculator' : 'Calculate Runaway'}</Button>
      {isClicked && <p>Estimated Runway: {runawayMonths} months</p>}
    </>
  );
};
