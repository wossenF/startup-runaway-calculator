...
interface ProjectedRevenue {
  ...
  useEffect(() => {
    ...
    setRunway((prevRunway) => {
      const newRunway = calculateRunway(userInput);
      if (typeof newRunway === 'number') { // Check if it's a number
        return newRunway;
      } else {
        return prevRunway; // Or handle the error here (e.g., console.warn)
      }
    });
